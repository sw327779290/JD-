...
from gen_feat2 import make_train_set
from gen_feat2 import make_test_set
from sklearn.model_selection import train_test_split
import xgboost as xgb
from gen_feat2 import report
# pip install -U scikit-learn
# http://stackoverflow.com/questions/40704484/iTailing Spacemporterror-no-module-named-model-selection
...
def xgboost_make_submission():
    ...
    print('------>Gernearting training_set ')
    ...
    bst=xgb.train(list(param.items()), dtrain, num_round, evallist)
    print('------>Gernearting testing_set ')
    ...
    pred = sub_user_index[sub_user_index['label'] >= 0.031]
...
def xgboost_cv():
    ...
    dtrain=xgb.DMatrix(X_train, label=y_train)
    dtest=xgb.DMatrix(X_test, label=y_test)
    param = {'max_depth': 10, 'eta': 0.05, 'silent': 1, 'objective': 'binary:logistic','eval_metric':'logloss'}
    num_round = 4000
    param['nthread'] = 4
    param['eval_metric'] = "auc"
    plst = param.items()
    ...
    print('------>Gernearting testing_set ')
    sub_user_index, sub_trainning_date, sub_label = make_train_set(sub_start_date, sub_end_date, sub_test_start_date, sub_test_end_date)
